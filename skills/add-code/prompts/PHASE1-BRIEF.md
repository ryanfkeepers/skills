...
1. `contract.go` — all exported interfaces and type declarations: `[INTERFACES]`
   This file is permanent. Phase 2 will not modify it. It is the source for mock
...
2. Stub `.go` files — exported function and method signatures with `panic("not
   implemented")` bodies. Phase 2 will replace the stub bodies with real implementations.
